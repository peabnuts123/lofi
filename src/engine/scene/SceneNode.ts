...
export abstract class SceneNode1 {
  private scene: Scene;
  public name: string;
  private _position: ObservedVector3;
  private _rotation: ObservedVector3;
  private _scale: ObservedVector3;

  private _parent: SceneNode1 | undefined;
  private children: SceneNode1[];

  private _worldMatrix: mat4 = mat4.create();
  private worldMatrixIsDirty: boolean = true;
  private _absolutePosition: ObservedVector3;
  private absolutePositionIsDirty: boolean = true;
  private _absoluteRotation: ObservedVector3;
  private absoluteRotationIsDirty: boolean = false;
  private _absoluteScale: ObservedVector3;
  ...
  public constructor(scene: Scene, name: string) {
    ...
    this._rotation = new ObservedVector3(0, 0, 0, () => {
      this.markWorldMatrixDirty();
    ...
    this._absolutePosition = new DirtyVector3(0, 0, 0,
      () => this.absolutePositionIsDirty,
      () => this.recomputeAbsolutePosition(),
      () => this.recomputeLocalPositionFromParent());
    this._absoluteRotation = new DirtyVector3(0, 0, 0,
      () => this.absoluteRotationIsDirty,
      () => this.recomputeAbsoluteRotation(),
      () => this.recomputeLocalRotationFromParent());
    this._absoluteScale = new DirtyVector3(1, 1, 1,
      () => this.absoluteScaleIsDirty,
      () => this.recomputeAbsoluteScale(),
      () => this.recomputeLocalScaleFromParent());
   ...
   */
  public addChild(child: SceneNode1): void {
   ...
   */
  public removeChild(child: SceneNode1): void {
   ...
   */
  public forEachChild(fn: (child: SceneNode1) => void): void {
  ...
  private recomputeWorldMatrix(): void {
    ...
    quat.fromEuler(this._rotationQuatTmp, this.absoluteRotation.x, this.absoluteRotation.y, this.absoluteRotation.z);
  ...
  private recomputeAbsolutePosition(): void {
    if (this.parent) {
      ...
      // @TODO evaporate `fromEuler` with native Rotation type
      quat.fromEuler(this._rotationQuatTmp, this.parent.absoluteRotation.x, this.parent.absoluteRotation.y, this.parent.absoluteRotation.z);
  ...
  private recomputeLocalPositionFromParent(): void {
    if (this.parent !== undefined) {
      ...
      // @TODO Evaporate by native Rotation type - can this type cache rotation inverse?
      quat.fromEuler(this._rotationQuatTmp, this.parent.absoluteRotation.x, this.parent.absoluteRotation.y, this.parent.absoluteRotation.z);
      quat.conjugate(this._rotationQuatTmp, this._rotationQuatTmp);
      vec3.transformQuat(this._positionVecTmp, this._positionVecTmp, this._rotationQuatTmp);
  ...
  private recomputeAbsoluteRotation(): void {
    if (this.parent) {
      ...
      this._absoluteRotation.setValue(
        this.parent.absoluteRotation.add(this.rotation),
        false,
      );
    } else {
      this._absoluteRotation.setValue(
  ...
  private recomputeLocalRotationFromParent(): void {
    if (this.parent !== undefined) {
      // Convert to local
      // @TODO is subtraction the right operation here?
      this.rotation = this.absoluteRotation.subtract(this.parent.absoluteRotation);
    } else {
      // No parent - absolute is the same as local
  ...
  public get parent(): SceneNode1 | undefined {
    return this._parent;
  }
  private set parent(value: SceneNode1) {
  ...
  public set rotation(value: Vector3) { this._rotation.setValue(value); }
  ...
  public set absoluteRotation(value: Vector3) { this._absoluteRotation.setValue(value); }
...
export abstract class SceneNode2 {
  private scene: Scene;
  public name: string;
  private _position: ObservedVector3;
  ...
  private _parent: SceneNode2 | undefined;
  ...
  public constructor(scene: Scene, name: string) {
    ...
    this._rotation = new Rotation(`${this.name}:local`, {
      onChange: () => {
        this.markWorldMatrixDirty();
        this.markAbsoluteRotationDirty();
      },
    ...
    this._absoluteRotation = new Rotation(`${this.name}:absolute`, {
   ...
   * @throws {Error} If the child already has a different parent
   */
  public addChild(child: SceneNode2): void {
    if (child.parent !== undefined) {
      throw new Error(`Cannot add node '${child.name}' as child of node '${this.name}': It is already the child of another node: '${child.parent.name}'`);
    } else if (this.children.some((existingChild) => existingChild === child)) {
      console.warn(`Tried to add node '${child.name}' as child of node '${this.name}' but it is already a child of this node`);
   ...
   * @param child - The scene node to remove
   */
  public removeChild(child: SceneNode2): void {
    const index = this.children.indexOf(child);
    if (index < 0) {
      console.warn(`Tried to remove node '${child.name}' from children of node '${this.name}' but it is not a child of this node`);
    } else {
   ...
   * @param fn - The callback function to execute for each child
   */
  public forEachChild(fn: (child: SceneNode2) => void): void {
    for (const child of this.children) {
      fn(child);
    }
  }
  ...
  private recomputeLocalPositionFromParent(): void {
    ...
    if (this.parent !== undefined) {
      ...
      // 2. "Unrotate" by inverse of parent's absolute rotation
      quat.set(this._rotationQuatTmp, this.parent.absoluteRotation.qConjugate.x, this.parent.absoluteRotation.qConjugate.y, this.parent.absoluteRotation.qConjugate.z, this.parent.absoluteRotation.qConjugate.w)
      vec3.transformQuat(this._positionVecTmp, this._positionVecTmp, this._rotationQuatTmp);
  ...
  public get parent(): SceneNode2 | undefined {
    return this._parent;
  }
  private set parent(value: SceneNode2) {
    const existingParent = this._parent;
    // Remove node from parent's children
    if (existingParent !== undefined) {
      existingParent.removeChild(this);
