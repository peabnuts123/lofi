import { DirtyVector3, ObservedVector3, Vector3 } from "./vector";
...
export class Transform<T extends TransformNodeTarget> {
  ...
  private recomputeLocalScaleFromParent(): void {
    if (this.parent !== undefined) {
      ...
      if (Math.abs(this.parent.absoluteScale.x) <= Number.EPSILON) {
        console.warn(`Cannot set absolute scaling to '${this.absoluteScale}' for node '${this.node.name}' as its parent(s) scaling.x is currently 0. Its local scaling.x will be set to 1. This will produce unexpected results when this node's parent(s) scale returns to a non-zero value.`);
      } else {
        newScale.x = this.absoluteScale.x / this.parent.absoluteScale.x;
      }
      /* Y */
      if (Math.abs(this.parent.absoluteScale.y) <= Number.EPSILON) {
        console.warn(`Cannot set absolute scaling to '${this.absoluteScale}' for node '${this.node.name}' as its parent(s) scaling.y is currently 0. Its local scaling.y will be set to 1. This will produce unexpected results when this node's parent(s) scale returns to a non-zero value.`);
      } else {
        newScale.y = this.absoluteScale.y / this.parent.absoluteScale.y;
      }
      /* Z */
      if (Math.abs(this.parent.absoluteScale.z) <= Number.EPSILON) {
        console.warn(`Cannot set absolute scaling to '${this.absoluteScale}' for node '${this.node.name}' as its parent(s) scaling.z is currently 0. Its local scaling.z will be set to 1. This will produce unexpected results when this node's parent(s) scale returns to a non-zero value.`);
      } else {
  ...
  public get parent(): Transform<T> | undefined {
    return this._parent;
  }
